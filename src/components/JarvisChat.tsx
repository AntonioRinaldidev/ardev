...
const JarvisChat: React.FC<JarvisChatProps> = ({ jarvisData }) => {
	...
	const getConnectionConfig = () => {
		switch (connectionState) {
			case 'connected':
				return { text: 'System Online', class: 'connected' };
			case 'connecting':
				return { text: 'Initializing...', class: 'connecting' };
			case 'error':
				return { text: 'Connection Failure', class: 'disconnected' };
			default:
				...
				<AnimatePresence mode='popLayout'>
					{messages.map((message: any) => (
						<motion.div
							...
							{message.sender === 'user' ? (
								<div className='user-command'>{message.content}</div>
							) : (
			...
			<div className='terminal-input'>
				<div className='input-container'>
					...
					<button
						...
						<FaArrowUp />{' '}
						{/* Icona freccia verso l'alto (stile ChatGPT/Gemini) */}
					</button>
				</div>
			</div>
		</div>
