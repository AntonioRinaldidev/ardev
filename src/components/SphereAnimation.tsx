...
// 1. Accettiamo la prop
interface SphereProps {
    isThinking?: boolean;
}

const SphereAnimation: React.FC<SphereProps> = ({ isThinking = false }) => {
    const root = useRef<HTMLDivElement | null>(null);
    const sphereRef = useRef<HTMLDivElement | null>(null);
    
    // Riferimento per controllare l'animazione corrente
    ...
    useEffect(() => {
        ...
        const CONFIG = {
            idle: {
                color: 'rgba(124, 58, 237, 1)', // Viola (Tuo Third Color)
                duration: 2000,
                scale: 1
            },
            thinking: {
                color: 'rgba(0, 185, 255, 1)',  // Ciano (Tuo Link Color - Colore Jarvis)
                duration: 600,                  // Molto più veloce!
                scale: 1.1
            }
        };
